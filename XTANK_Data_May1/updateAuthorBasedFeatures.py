...
try:    
    ...
    conn = MySQLdb.connect (host = "localhost",user = "root",passwd = "iltwat", db = "X1Diggv2" , connect_timeout =300)
...
cursor.execute("SELECT digg_id FROM diggFeatures")
                
row = cursor.fetchall ()
total_rows = cursor.rowcount
print total_rows
    
for i in range(2485, total_rows):
    if(i%10 == 0):
	percentComplete  = i/total_rows
	print percentComplete, "% updated"   	 
    story_id = row[i][0]
    ...
    if data!=None and data['count'] != 0:
        ...
        for index in range(0 , data['count']):
            ...
            digg_id.encode('utf8')
	    diggs_of_story = data['stories'][index]['diggs']
        ...
        #=======================================================================
        try :    
            cursor2.execute("UPDATE diggFeatures SET fofCount =%d, authorDiggs=%d, authorComments=%d, authorFans=%d, authorFollows =%d, authorGender='%s' , authorSubmissions=%d WHERE digg_id = '%s'" % ( fof , diggs , comments , followers , following , gender , submissions, digg_id))
